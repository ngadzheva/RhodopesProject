...
userRouter.get('/info', auth_1.auth, function (request, response) {
    response.status(200).send({ success: true, data: login_1.user.viewProfileInfo() });
});
...
userRouter.post('/uploadImage', auth_1.auth, function (request, response) {
    ...
    form.on('end', function () {
        setTimeout(function () {
            response.status(200).send({ success: true, data: login_1.user.getImage() });
        }, 1000);
    });
    form.parse(request);
});
userRouter.get('/:listType', auth_1.auth, function (request, response) {
    ...
    if (listType === 'favorite') {
        response.status(200).send({ success: true, data: login_1.user.getFavoriteList() });
    }
    else if (listType === 'visited') {
        response.status(200).send({ success: true, data: login_1.user.getVisitedList() });
    }
    else if (listType === 'wantToVisit') {
        response.status(200).send({ success: true, data: login_1.user.getWantToVisitList() });
    }
    else if (listType === 'tripplans') {
        response.status(200).send({ success: true, data: login_1.user.listUserTrips() });
    }
    else {
